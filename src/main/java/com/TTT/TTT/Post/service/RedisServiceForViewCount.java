...
//    여기서부터는 프로젝트 조회용
    private static final String PROJECT_VIEW_COUNT_PREFIX = "project-viewCount-";
    private static final String PROJECT_VIEW_LOCK_PREFIX = "project-viewLock-";
   ...
   //프로젝트용
    public void increaseViewCountForProject(Long projectId,String userId){
        String key = PROJECT_VIEW_COUNT_PREFIX + projectId;
        String lockKey = PROJECT_VIEW_LOCK_PREFIX + projectId +"-" + userId;
  ...
  //프로젝트용
    public int getViewCountForProject(Long postId){
        String key = PROJECT_VIEW_COUNT_PREFIX + postId;
        String countStr = redisTemplate.opsForValue().get(key); //레디스에 해당 키가 없으면 countStr = null이 됨
        return countStr == null ? 0 : Integer.parseInt(countStr);
    }
    ...
    //프로젝트용
    public Map<Long,Integer> getAllViewCountForProjectForRdb(){
        Set<String>keys = redisTemplate.keys(PROJECT_VIEW_COUNT_PREFIX+"*"); //
        Map<Long,Integer> viewCounts = new HashMap<>();
